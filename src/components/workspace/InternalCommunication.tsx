...
import { Download, FileText, Image as ImageIcon, Loader2, MessageSquare, Paperclip, Phone, Plus, Search, Send, Users, Video, X } from "lucide-react";
...
export function InternalCommunication({ business, profile, members }: { business: Business | null; profile: any; members: WorkspaceMembership[] }) {
  ...
  const [call, setCall] = useState<{ mode: "voice" | "video"; incoming?: any } | null>(null);
  ...
  useEffect(() => { const timer = window.setInterval(async () => { if (!identity || call) return; const response = await fetch(`/api/internal-calls?userId=${identity}`, { cache: "no-store" }); const data = await response.json(); if (response.ok && data.incoming) { const belongs = sessions.some((item) => item.$id === data.incoming.chatSessionId); if (belongs) { setSelected(data.incoming.chatSessionId); setCall({ mode: data.incoming.mode === "video" ? "video" : "voice", incoming: data.incoming }); } } }, 2500); return () => window.clearInterval(timer); }, [call, identity, sessions]);
  ...
  async function shareFile(file?: File) {
    ...
    setUploading(true);
    const form = new FormData(); form.set("file", file); form.set("sessionId", selected); form.set("userId", identity);
    const response = await fetch("/api/internal-chat/attachment", { method: "POST", body: form }); const data = await response.json();
    if (!response.ok) toast.error(data.error || "File could not be shared."); else { setMessages((current) => [...current, data.message]); void loadSessions(); }
    setUploading(false); if (fileInput.current) fileInput.current.value = "";
  }
  const fileUrl = (message: any) => `/api/internal-chat/attachment?sessionId=${encodeURIComponent(selected)}&userId=${encodeURIComponent(identity)}&fileId=${encodeURIComponent(readString(message,"fileId"))}`;
  ...
  return <div className="space-y-5">
    ...
    <section className="grid h-[calc(100vh-12rem)] min-h-[520px] grid-cols-1 overflow-hidden rounded-lg border bg-white md:grid-cols-[280px_1fr]">
      ...
      <div className="flex min-w-0 flex-col">{selected ? <><div className="flex items-center justify-between border-b px-4 py-3"><div><p className="font-bold">{otherName}</p><p className="text-xs text-slate-500">Private business conversation</p></div><div className="flex gap-1"><button onClick={() => setCall({mode:"voice"})} className="grid size-9 place-items-center rounded-md text-slate-600 hover:bg-slate-100" title="Voice call"><Phone size={17}/></button><button onClick={() => setCall({mode:"video"})} className="grid size-9 place-items-center rounded-md text-slate-600 hover:bg-slate-100" title="Video call"><Video size={18}/></button></div></div><div className="flex-1 space-y-3 overflow-y-auto bg-slate-50/50 p-4">{messages.map((message) => { const mine = readString(message,"senderId") === identity; const type=readString(message,"messageType"); const url=readString(message,"fileId")?fileUrl(message):""; return <div key={message.$id} className={`flex ${mine ? "justify-end" : "justify-start"}`}><div className={`max-w-[78%] overflow-hidden rounded-lg text-sm ${mine ? "bg-blue-600 text-white" : "border bg-white text-slate-800"}`}>{type==="image"?<a href={url} target="_blank" rel="noreferrer"><img src={url} alt={readString(message,"fileName")||"Shared image"} className="max-h-72 w-full min-w-52 object-cover"/></a>:null}<div className="px-3 py-2">{!mine && <p className="mb-1 text-[10px] font-bold text-blue-600">{readString(message,"senderName")}</p>}{type==="file"?<a href={url} target="_blank" rel="noreferrer" className="flex min-w-56 items-center gap-3"><span className={`grid size-10 place-items-center rounded-md ${mine?"bg-white/15":"bg-blue-50 text-blue-600"}`}><FileText size={19}/></span><span className="min-w-0 flex-1"><strong className="block truncate">{readString(message,"fileName")}</strong><small className={mine?"text-blue-100":"text-slate-500"}>{Math.max(1,Math.round(Number(readString(message,"fileSize"))/1024))} KB</small></span><Download size={17}/></a>:type!=="image"?<p>{readString(message,"messageText")}</p>:<p className="flex items-center gap-1 text-xs"><ImageIcon size={13}/> {readString(message,"fileName")}</p>}<p className={`mt-1 text-[10px] ${mine ? "text-blue-100" : "text-slate-400"}`}>{new Date(readString(message,"createdAt") || message.$createdAt).toLocaleString()}</p></div></div></div>})}</div><div className="flex items-center gap-2 border-t p-3"><input ref={fileInput} type="file" className="hidden" accept=".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.zip" onChange={(event)=>void shareFile(event.target.files?.[0])}/><button onClick={()=>fileInput.current?.click()} disabled={uploading} className="grid size-10 place-items-center rounded-md text-slate-500 hover:bg-slate-100 disabled:opacity-50" title="Share a file">{uploading?<Loader2 className="size-4 animate-spin"/>:<Paperclip size={18}/>}</button><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void send()} placeholder="Write an internal message" className="h-10 flex-1 rounded-md border px-3 text-sm"/><button onClick={() => void send()} disabled={!input.trim()} className="grid size-10 place-items-center rounded-md bg-blue-600 text-white disabled:opacity-40"><Send size={17}/></button></div></> : <div className="grid flex-1 place-items-center text-center"><div><MessageSquare className="mx-auto size-9 text-slate-300"/><p className="mt-3 font-bold">Select a conversation</p><p className="mt-1 text-sm text-slate-500">Team and partner messages stay inside this business.</p></div></div>}</div>
    </section>
    ...
    {call && selected ? <InternalCallDialog chatSessionId={selected} userId={identity} otherName={otherName} mode={call.mode} incoming={call.incoming} onClose={()=>setCall(null)}/> : null}
  </div>;
}
