...
export function normalizeStatus(value?: string): ServiceRequest["status"] {
  ...
  if (["cancelled", "canceled", "rejected"].includes(status))
    return "cancelled";
  if (
    ["awaiting_payment", "payment_pending", "pending_payment"].includes(status)
  )
...
export function userIdentity(
  profile?: { userId?: string; customerId?: string; phone?: string } | null,
) {
  return {
...
export function toServiceRequest(doc: any): ServiceRequest {
  const createdAt =
    readString(doc, "createdAt") || doc?.$createdAt || new Date().toISOString();
  const updatedAt =
    readString(doc, "updatedAt") || doc?.$updatedAt || createdAt;
  const serviceType =
    readString(doc, "serviceType") || readString(doc, "type") || "Service";
  const title =
  ...
  return {
    ...
    siteAddress:
      ...
      [readString(doc, "city"), readString(doc, "state")]
        .filter(Boolean)
        .join(", "),
    customerId:
      readString(doc, "customerId") ||
      readString(doc, "user_id") ||
      readString(doc, "requestorUserId"),
    customerName:
    ...
    assignedTo:
      readString(doc, "assignedTo") ||
      readString(doc, "partnerId") ||
      undefined,
    assignedBusinessId:
      readString(doc, "assignedBusinessId") ||
      readString(doc, "businessId") ||
      undefined,
    estimatedCost:
      readInt(doc, "estimatedCost") || readInt(doc, "amount") || undefined,
    finalCost: readInt(doc, "finalCost") || undefined,
    scheduledAt:
      readString(doc, "scheduledAt") ||
      readString(doc, "scheduledDate") ||
      undefined,
...
export function toChatSession(doc: any, currentUserId?: string): ChatSession {
  ...
  const otherName =
    ...
    businessName;
  const chatType =
    readString(doc, "chatType") || readString(doc, "type") || "business";

  return {
    $id: doc.$id,
    type: chatType.includes("support")
      ? "support"
      : chatType.includes("partner")
        ? "partner"
        : "business",
    ...
    businessName,
    requestId:
      readString(doc, "requestId") ||
      readString(doc, "serviceRequestId") ||
      undefined,
    amcId: readString(doc, "amcId") || undefined,
    lastMessage: readString(doc, "lastMessage"),
    lastMessageAt:
      readString(doc, "lastMessageTime") ||
      readString(doc, "updatedAt") ||
      doc.$updatedAt,
...
export function toChatMessage(doc: any): ChatMessage {
  return {
    ...
    type: (readString(doc, "messageType") ||
      readString(doc, "type") ||
      "text") as ChatMessage["type"],
    content:
      readString(doc, "messageText") ||
      readString(doc, "text") ||
      readString(doc, "content"),
    mediaUrl: readString(doc, "mediaUrl") || undefined,
    createdAt:
      readString(doc, "timestamp") ||
      readString(doc, "createdAt") ||
      doc.$createdAt,
  };
}
...
export function toActivityEvent(doc: any, userId = ""): ActivityEvent {
  const type =
    readString(doc, "type") || readString(doc, "notificationType") || "system";
  const normalizedType = type.includes("chat")
    ? "chat"
  ...
  return {
    $id: doc.$id,
    userId:
      readString(doc, "recipientUserId") || readString(doc, "userId") || userId,
    type: normalizedType,
    title: readString(doc, "title") || "Notification",
    description:
      readString(doc, "body") ||
      readString(doc, "message") ||
      readString(doc, "description"),
...
export function toFeedPost(doc: any, currentUserId = ""): FeedPost {
  ...
  const resolvedMediaType =
    ...
    mediaUrl.match(/\.(mp4|mov|webm|mkv)(\?|$)/i)
      ? "video"
  ...
  return {
    ...
    authorId:
      readString(doc, "user_id") ||
      readString(doc, "userId") ||
      readString(doc, "authorUserId") ||
      readString(doc, "authorId"),
    authorName:
      readString(doc, "author") ||
      readString(doc, "authorName") ||
      readString(doc, "name") ||
      "AMC MEP user",
    authorAvatar: readString(doc, "authorAvatar") || undefined,
    authorRole: (readString(doc, "authorRole") ||
      readString(doc, "role") ||
...
export function toMarketplaceItem(doc: any): MarketplaceItem {
  const mediaId =
    readString(doc, "mediaId") ||
    readString(doc, "imageId") ||
    readString(doc, "fileId");
  const image = mediaId
    ? fileViewUrl(BUCKETS.marketplace, mediaId)
    : readString(doc, "imageUrl");
  const price =
    readInt(doc, "price") || readInt(doc, "amount") || readInt(doc, "rate");

  return {
    $id: doc.$id,
    title:
      readString(doc, "displayTitle") ||
      readString(doc, "title") ||
      readString(doc, "name") ||
      "Marketplace listing",
    description:
      readString(doc, "description") || readString(doc, "details") || "",
    price,
    currency: readString(doc, "currency") || "INR",
    category:
      readString(doc, "category") ||
      readString(doc, "serviceType") ||
      "Services",
    images: image ? [image] : [],
    condition: (readString(doc, "condition") ||
      "new") as MarketplaceItem["condition"],
    sellerId:
      readString(doc, "providerId") ||
      readString(doc, "sellerId") ||
      readString(doc, "userId"),
    sellerName:
      readString(doc, "sellerName") ||
      readString(doc, "businessName") ||
      readString(doc, "providerName") ||
      "AMC MEP seller",
    sellerBusinessId: readString(doc, "businessId") || undefined,
    status: (readString(doc, "status") ||
      "active") as MarketplaceItem["status"],
    location:
      readString(doc, "location") || readString(doc, "city") || undefined,
    createdAt: readString(doc, "createdAt") || doc.$createdAt,
  };
}
...
export function toAmcRecord(doc: any): AMCRecord {
  const startDate =
    readString(doc, "amcStartDate") ||
    readString(doc, "startDate") ||
    readString(doc, "createdAt") ||
    doc.$createdAt;
  const endDate =
    readString(doc, "amcEndDate") ||
    readString(doc, "endDate") ||
    readString(doc, "scheduledAt") ||
    "";
  const now = Date.now();
  const end = endDate ? new Date(endDate).getTime() : 0;
  const derivedStatus =
    normalizeStatus(readString(doc, "status")) === "cancelled"
      ? "cancelled"
      : end && end < now
        ? "expired"
        : end && end - now < 1000 * 60 * 60 * 24 * 30
          ? "expiring_soon"
          : "active";
  ...
  return {
    ...
    title:
      readString(doc, "title") ||
      readString(doc, "serviceType") ||
      readString(doc, "amcType") ||
      "AMC contract",
    businessId:
      readString(doc, "businessId") || readString(doc, "assignedBusinessId"),
    businessName:
      readString(doc, "businessName") ||
      readString(doc, "providerName") ||
      "AMC MEP provider",
    ...
    amcType:
      readString(doc, "amcType") || readString(doc, "serviceType") || "AMC",
    startDate,
    endDate: endDate || startDate,
    cost:
      readInt(doc, "cost") ||
      readInt(doc, "estimatedCost") ||
      readInt(doc, "amount"),
    paymentStatus: (readString(doc, "paymentStatus") ||
      "pending") as AMCRecord["paymentStatus"],
    visitCount: readInt(doc, "visitCount") || readInt(doc, "completedVisits"),
    totalVisits: readInt(doc, "totalVisits") || readInt(doc, "visits") || 1,
    nextVisitDate:
      readString(doc, "nextVisitDate") ||
      readString(doc, "scheduledAt") ||
      undefined,
    siteAddress:
      readString(doc, "siteAddress") ||
...
export function toBusiness(doc: any): Business {
  ...
  return {
    $id: doc.$id,
    name:
      readString(doc, "businessName") || readString(doc, "name") || "Business",
    slug: readString(doc, "slug") || doc.$id,
    description:
      readString(doc, "description") || readString(doc, "businessType"),
    ...
    servicesEnabled:
      servicesEnabledValue === undefined || servicesEnabledValue === null
        ? true
        : readBool(doc, "servicesEnabled"),
    vendorEnabled: readBool(doc, "vendorEnabled"),
    status: (readString(doc, "status") || "active") as Business["status"],
    ownerId:
      readString(doc, "ownerId") ||
      readString(doc, "user_id") ||
      readString(doc, "ownerUserId"),
...
export function toPartnerProfile(doc: any): PartnerProfile {
  ...
  return {
    ...
    partnerType: (readString(doc, "partnerType") ||
      "service") as PartnerProfile["partnerType"],
    status: (readString(doc, "status") ||
      readString(doc, "partnerProgramStatus") ||
      "pending") as PartnerProfile["status"],
...
export function toPartnerAssignment(doc: any): PartnerAssignment {
  return {
    ...
    requestId:
      readString(doc, "requestId") || readString(doc, "serviceRequestId"),
    businessId: readString(doc, "businessId"),
    status: (readString(doc, "status") ||
      "assigned") as PartnerAssignment["status"],
...
export async function fetchRequests({
  ...
  async function pullBy(attribute: string, value?: string) {
    ...
    if (!trimmed) return;
    try {
      const resp = await appwrite.databases.listDocuments(
        DB_ID,
        COLLECTIONS.serviceRequests,
        [
          Query.equal(attribute, trimmed),
  ...
  return Array.from(merged.values())
    .filter((doc) => isVisibleRequest(doc))
    .filter((doc) => readString(doc, "type") !== "amc")
    .sort(
      (a, b) =>
...
export async function fetchAmcRecords({
  customerId,
  userId,
  phone,
  limit = 100,
  ...
  return Array.from(merged.values())
    .filter((doc) => isVisibleRequest(doc))
    .filter((doc) => {
      const type = readString(doc, "type").toLowerCase();
      const amcType = readString(doc, "amcType").trim();
      return type === "amc" || amcType.length > 0;
    })
    .sort((a, b) => {
      const ae = readString(a, "amcEndDate");
      const be = readString(b, "amcEndDate");
      if (!ae && !be)
        return (
          new Date(b.$createdAt).getTime() - new Date(a.$createdAt).getTime()
        );
...
export async function fetchChatSessions({
  ...
  const idCandidates = new Set(
    [customerId?.trim() ?? "", userId?.trim() ?? ""].filter((e) => e),
  );
  ...
  await pullBy("clientId", customerId);
  await pullBy("clientId", userId);
  await pullBy("participantUserId", userId);
  await pullBy("participantUserId", customerId);
  await pullBy("partnerUserId", userId);
  await pullBy("initiatorUserId", userId);
  await pullBy("initiatorUserId", customerId);
  if (cleanPhone) await pullBy("clientPhone", phone);
  const businessIds = new Set<string>();
  for (const id of [customerId, userId]) {
    if (!id?.trim()) continue;
    try {
      const rows = await appwrite.databases.listDocuments(
        DB_ID,
        COLLECTIONS.clients,
        [Query.equal("user_id", id.trim()), Query.limit(10)],
      );
      for (const row of rows.documents) {
        const raw = row.data?.businessIds;
        if (Array.isArray(raw)) {
          for (const v of raw) {
            const s = v?.toString().trim();
            if (s) businessIds.add(s);
          }
        }
      }
    } catch {}
  ...
  return Array.from(merged.values())
    .filter((s) => {
      ...
      if (chatType === "partner_direct" || chatType === "partner_internal")
        return false;
      if (
        chatType.includes("partner") &&
        !chatType.includes("client") &&
        !readString(s, "clientId")
      )
        return false;
      const counterpartyType = readString(s, "counterpartyType").toLowerCase();
      if (counterpartyType === "partner" && !readString(s, "clientId"))
        return false;
      if (idCandidates.size > 0) {
        const sessionIds = new Set(
          [
            readString(s, "clientId"),
            readString(s, "participantUserId"),
            readString(s, "initiatorUserId"),
          ].filter(Boolean),
        );
        const matchesId = [...sessionIds].some((id) => idCandidates.has(id));
        const matchesPhone =
          cleanPhone && readString(s, "clientPhone") === cleanPhone;
        if (!matchesId && !matchesPhone) return false;
      } else if (cleanPhone) {
        if (readString(s, "clientPhone") !== cleanPhone) return false;
      } else {
        return false;
      }
      return true;
...
export async function fetchChatMessages(sessionId: string, limit = 150) {
  ...
  return resp.documents.map((doc) => {
    ...
    if (mediaUrl && !mediaUrl.startsWith("http"))
      return { ...doc, mediaUrl: fileViewUrl(BUCKETS.chatMedia, mediaUrl) };
    return doc;
  });
}
...
export async function fetchFeed({
  limit = 20,
  cursorAfter,
}: { limit?: number; cursorAfter?: string } = {}) {
  ...
  return resp.documents.map((doc) => {
    const mediaUrl = readString(doc, "mediaUrl");
    if (mediaUrl && !mediaUrl.startsWith("http"))
      return { ...doc, mediaUrl: fileViewUrl(BUCKETS.feed, mediaUrl) };
    return doc;
  });
}
...
export async function toggleFeedLike(
  ...
  await appwrite.databases.updateDocument(DB_ID, COLLECTIONS.feed, postId, {
...
export async function addFeedComment({
  ...
  try {
    const post = await appwrite.databases.getDocument(
      DB_ID,
      COLLECTIONS.feed,
      postId,
    );
    const currentCount = readInt(post, "commentsCount");
    await appwrite.databases.updateDocument(DB_ID, COLLECTIONS.feed, postId, {
...
export async function fetchMarketplace({
  ...
  return resp.documents.filter(
    (doc) =>
      readString(doc, "displayTitle").trim() || readString(doc, "title").trim(),
  );
}
...
export async function fetchAssignments({
  ...
  businessId,
  limit = 100,
}: { partnerId?: string; businessId?: string; limit?: number } = {}) {
  const merged = new Map<string, any>();
  async function pullBy(attribute: string, value?: string) {
    ...
    try {
      const resp = await appwrite.databases.listDocuments(
        ...
        [
          Query.equal(attribute, value.trim()),
          Query.orderDesc("$createdAt"),
          Query.limit(limit),
        ],
      );
...
export async function fetchPartnerProfile(userId?: string) {
  ...
  try {
    const rows = await appwrite.databases.listDocuments(
      ...
      [Query.equal("user_id", userId.trim()), Query.limit(1)],
    );
...
export async function fetchNotifications({
  ...
  const resp = await appwrite.databases.listDocuments(
    DB_ID,
    COLLECTIONS.notificationInbox,
    [
      Query.equal("recipientUserId", userId.trim()),
      Query.orderDesc("$createdAt"),
      Query.limit(limit),
    ],
  );
...
export async function fetchClientProfile(userId: string) {
  ...
  try {
    const rows = await appwrite.databases.listDocuments(
      DB_ID,
      COLLECTIONS.clients,
      [Query.equal("customerId", userId.trim()), Query.limit(1)],
    );
    if (rows.documents.length > 0) return rows.documents[0];
  } catch {}
  return null;
...
export function validatePublicUserId(value: string) {
  ...
  if (normalized.length < 4 || normalized.length > 24) {
    return {
      valid: false,
      value: normalized,
      message: "Use 4 to 24 characters.",
    };
  }
...
export async function isPublicUserIdAvailable(
  ...
  const resp = await appwrite.databases.listDocuments(
    DB_ID,
    COLLECTIONS.clients,
    [Query.equal("customerId", normalized), Query.limit(2)],
  );
...
export async function fetchBusinesses({
  ...
  return Array.from(merged.values()).filter(
    (doc) => readBool(doc, "isActive") !== false,
  );
}
...
export async function fetchBusinessMemberships(businessId: string) {
  const resp = await appwrite.databases.listDocuments(
    DB_ID,
    COLLECTIONS.businessMemberships,
    [
  ...
  for (const attribute of ["customerId", "email", "phone", "name"]) {
    try {
      const response = await appwrite.databases.listDocuments(
        DB_ID,
        COLLECTIONS.clients,
        [Query.search(attribute, value), Query.limit(limit)],
      );
...
export async function updateWorkspaceMember(
  memberId: string,
  data: Partial<
    Pick<WorkspaceMembership, "role" | "permissions" | "status" | "onDuty">
  >,
) {
  return appwrite.databases.updateDocument(
    DB_ID,
...
export async function fetchMembershipsForUser(userId: string) {
  ...
  const resp = await appwrite.databases.listDocuments(
    DB_ID,
    COLLECTIONS.businessMemberships,
    [Query.equal("userId", userId.trim()), Query.limit(100)],
  );
  return resp.documents.filter(
    (doc) => (readString(doc, "status") || "active") === "active",
  );
}

export async function fetchBusinessesByIds(businessIds: string[]) {
  const uniqueIds = Array.from(
    new Set(businessIds.map((id) => id.trim()).filter(Boolean)),
  );
...
export type BusinessRecordTable =
  | "projects"
  | "tasks"
  | "quotations"
  | "invoices"
...
export async function fetchBusinessRecords(
  ...
  if (userId) {
    ...
    if (!response.ok)
      throw new Error(result.error || "Records could not be loaded.");
...
export function recordCreator(doc: any) {
  return {
    ...
    name:
      readString(doc, "createdByName") || readString(doc, "createdByAdminName"),
  };
}
