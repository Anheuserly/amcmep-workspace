...
if (!key) {
  console.log("APPWRITE_API_KEY is not available; skipping the remote communication schema check.");
  process.exit(0);
}
const db = new Databases(new Client().setEndpoint(endpoint).setProject(projectId).setKey(key));
const storage = new Storage(new Client().setEndpoint(endpoint).setProject(projectId).setKey(key));
...
for (const [keyName, size, required, array, fallback] of [
  ...
  ["fileId", 100, false, false, ""], ["fileName", 255, false, false, ""], ["fileMime", 150, false, false, ""], ["fileSize", 30, false, false, ""],
...
for (const collection of ["internal_call_sessions", "internal_call_candidates"]) {
  await ensureCollection(collection, collection === "internal_call_sessions" ? "Internal call sessions" : "Internal call candidates");
}
for (const [keyName, size, required, array, fallback] of [
  ["chatSessionId", 100, true, false], ["businessId", 100, true, false], ["callerId", 100, true, false], ["callerName", 150, true, false],
  ["calleeId", 100, true, false], ["calleeName", 150, true, false], ["mode", 20, true, false], ["status", 30, true, false],
  ["offerSdp", 50000, true, false], ["answerSdp", 50000, false, false, ""], ["createdAt", 40, true, false], ["answeredAt", 40, false, false, ""], ["endedAt", 40, false, false, ""],
]) await ensureString("internal_call_sessions", keyName, size, required, array, fallback);
for (const [keyName, size, required, array, fallback] of [
  ["callId", 100, true, false], ["senderId", 100, true, false], ["candidate", 4000, true, false], ["sdpMid", 100, false, false, ""], ["sdpMLineIndex", 20, false, false, ""], ["createdAt", 40, true, false],
]) await ensureString("internal_call_candidates", keyName, size, required, array, fallback);
await wait();
await ensureIndex("internal_call_sessions", "callee_status_created", ["calleeId", "status", "createdAt"]);
await ensureIndex("internal_call_sessions", "chat_created", ["chatSessionId", "createdAt"]);
await ensureIndex("internal_call_candidates", "call_created", ["callId", "createdAt"]);

try { await storage.getBucket("internal_chat_files"); }
catch { await storage.createBucket("internal_chat_files", "Internal communication files", [], false, true, 20 * 1024 * 1024, ["jpg","jpeg","png","gif","webp","pdf","doc","docx","xls","xlsx","csv","txt","zip"], "none", true, true); }
